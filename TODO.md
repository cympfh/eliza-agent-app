...
## [x] Chat Response に tool フィールドが追加 [2026-02-24 完了]

Eliza Agent Server はオプショナルで `tool: Option<Vec<(JSON, JSON)>>` を返す。
`ChatResponse` の `tool` フィールドとして受け取るが、GUI にも Grok への送信履歴にも使用しない。
受け取るだけで無視する。

## [x] Eliza Agent Server に system prompt は投げない [2026-02-25 完了]

設定では `settings_custom_prompt` としてある prompt はサーバ側で挿し込まれることになったので、 クライアントからは投げない。
設定項目からも消す。

### 実装内容
- `config.rs`: `system_prompt` フィールドを削除（`custom_prompt` は Whisper 用なので残す）
- `eliza.rs`: `system_prompt` フィールドと引数を削除。`send_message()` / `save_memory()` で messages に system メッセージを含めなくなった
- `main.rs`: `settings_system_prompt` フィールド、UI の "System Prompt:" 入力欄、設定保存・キャンセル時の同期処理をすべて削除
